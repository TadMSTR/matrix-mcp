...
- **GitHub private disclosure:** Use the [Security tab](https://github.com/TadMSTR/matrix-mcp/security/advisories/new) to submit a private advisory.
- **Email:** Send a description to `security.i9v75@8alias.com` with the subject line `[matrix-mcp] Security Report`.
...
- Matrix credential exposure through tool responses or error messages
- Room access bypass allowing reads or writes to rooms outside the bot's intended scope
- Message injection that bypasses intended content or room filtering
- Dependency vulnerabilities with a plausible exploitation path in matrix-mcp's usage
